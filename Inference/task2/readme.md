...
### 数值陷阱：为什么代码里要处理 `-inf - -inf`？

因果掩码会让某些 block 中的全部候选位置无效。这时 `scores` 可能全为 `-inf`，块最大值也为 `-inf`。若直接计算 `scores - m_new`，就会得到 `-inf - -inf = NaN`。教学实现显式使用 `torch.where`：无效位置的指数输入被替换为负无穷，其指数恰为零；旧状态在尚未见到有效分数时也将缩放系数设为零。这个分支不是优化技巧，而是让掩码语义与数值稳定性同时成立的必要防线。
...
4. 把本文“未归一化分子除以归一化分母”的写法改为每块直接维护归一化输出，再证明两种形式的数学等价性。
